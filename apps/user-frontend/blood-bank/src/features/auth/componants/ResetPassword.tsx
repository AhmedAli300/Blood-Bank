...
import { useResetPassword } from '@/features/auth/hooks/useResetPasword';
      ...
      <div className="bg-white rounded-[2rem] shadow-2xl flex flex-col md:flex-row overflow-hidden max-w-4xl w-full border border-gray-100">
        ...
        <div className="md:w-2/3  p-8 md:p-14 md:py-8 bg-white">
          ...
          <div className="flex border-b border-gray-100 mb-10 relative ">
            <button className="flex-1 text-center pb-4 text-(--primary-color) font-bold border-b-2 border-(--primary-color) transition-all">
              رمز التحقيق
            </button>
          ...
          <form onSubmit={onSubmit} className="space-y-6">
            ...
            {/* زر تسجيل الدخول */}
            <Button disabled={loading}   variant="secondary" size="lg" className="w-full !text-white bg-(--primary-color) hover:bg-(--primary-brand) cursor-pointer  font-bold py-5 text-md+ rounded-2xl flex items-center justify-center gap-3 transition-all shadow-lg shadow-green-900/10 group">
              {loading ?  (<span className="flex items-center gap-2"> <Spinner />جاري  التغير</span> ) : ( "تأكيد")}

                
            </Button>
            {/* <Link href={"home"} className="w-full bg-(--primary-color) hover:bg-(--primary-brand) text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-3 transition-all shadow-lg shadow-green-900/10 group">
             تأكيد
