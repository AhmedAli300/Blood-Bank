...
class NotificationController {
  ...
  static async cleanupAllNotifications(req, res) {
    try {
      if (req.user.role !== 'SUPER_ADMIN') {
        return res.status(403).json({
          error: 'Only SUPER_ADMIN can perform cleanup'
        })
      }

      const NotificationModel = require('./notification.model')

      const beforeCount = await NotificationModel.getCount()
      const deletedCount = await NotificationModel.deleteAll()

      res.status(200).json({
        message: 'All notifications cleaned up successfully',
        data: {
